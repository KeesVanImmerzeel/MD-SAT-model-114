library DSmodel114;
  {-Berekening nat-, droogte- en totale schade aan landbouwgewassen
    conform Waternood 2002, benaderd met analytische functies }
...
Const
  cModelID      = 114;  {-Uniek modelnummer}
  ...
  cTb_coeff_Mais_Natschade     = 5; {-Coefficienten in regressiefunctie mais natschade}
  cTb_Coeff_Gras_Droogteschade = 6; {-Coefficienten in regressiefunctie grasland droogteschade}
  cTb_Coeff_Bouw_Droogteschade = 7; {-Coefficienten in regressiefunctie bouwland droogteschade}
  cTb_Coeff_Mais_Droogteschade = 8; {-Coefficienten in regressiefunctie mais droogteschade}
  ...
  cTb_Landgebruik     = 0; {1=Grasland; 2=Bouwland; 3=Mais}
  cTb_Bodemtype       = 1; {1-70, (HELP indeling 2002)}
  cTb_GHG             = 2; {-m-mv}
  cTb_GLG             = 3; {-m-mv}
      ...
      case landgebruik of
        cGrasland:
          with Items[ cTb_coeff_Gras_Natschade ] do begin
            ...
            F_coeff := GetValue( bodemtype, 6 );
            G_coeff := GetValue( bodemtype, 7 );
            H_coeff := GetValue( bodemtype, 8 );
          end;
        cBouwland:
        ...
        cMais:
          with Items[ cTb_coeff_Mais_Natschade ] do begin
            A_coeff := GetValue( bodemtype, 1 ); {row, column}
            B_coeff := GetValue( bodemtype, 2 );
            C_coeff := GetValue( bodemtype, 3 );
            D_coeff := GetValue( bodemtype, 4 );
      ...
      case landgebruik of
        cGrasland:
          with Items[ cTb_Coeff_Gras_Droogteschade ] do begin
            A_coeff := GetValue( bodemtype, 1 ); {row, column}
            B_coeff := GetValue( bodemtype, 2 );
            {C_coeff := GetValue( bodemtype, 3 ); niet gebruikt}
            D_coeff := GetValue( bodemtype, 3 );
            E_coeff := GetValue( bodemtype, 4 );
            F_coeff := GetValue( bodemtype, 5 );
          end;
        cBouwland:
        ...
        cMais:
          with Items[ cTb_Coeff_Mais_Droogteschade ] do begin
            A_coeff := GetValue( bodemtype, 1 ); {row, column}
            B_coeff := GetValue( bodemtype, 2 );
            {C_coeff := GetValue( bodemtype, 3 ); niet gebruikt}
            D_coeff := GetValue( bodemtype, 3 );
...
begin {-Function SetKeyAndParValues}
  ...
  NatschadePerc := max( max( F_coeff + G_coeff * GLG, H_coeff ), min( 100,
                   exp((GHG-GLG)/E_coeff) * ( A_coeff + B_coeff * Power(GHG + C_coeff,-D_coeff) ) ) );
  ...
  DroogteschadePerc := max( F_coeff, min( 100,
    F_coeff + A_coeff * ( 1 - 1 / ( 1 + Power( B_coeff*(max(GLG+E_coeff*GHG,0.01)),D_coeff ) ) ) ) );
